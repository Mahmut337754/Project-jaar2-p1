...
<body>
    <div class="container">
        ...
        <!-- Knoppen om alle verkopers te wissen of terug te halen -->
        <form action="{{ route('verkopers.wisAlles') }}" method="POST" style="display:inline;">
            @csrf
            <button type="submit">Wis alle verkopers</button>
        </form>

        <form action="{{ route('verkopers.herstelAlles') }}" method="POST" style="display:inline;">
            @csrf
            <button type="submit">Herstel alle verkopers</button>
        </form>

        <!-- Bericht tonen als er geen verkopers zijn -->
        @if(isset($message))
            <p>{{ $message }}</p>
        @else
