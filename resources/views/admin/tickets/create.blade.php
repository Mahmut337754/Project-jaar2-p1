                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <select name="event_id" id="event_id" 
                                    ...
                                    @foreach($events as $eventOption)
                                        <option value="{{ $eventOption->id }}" 
                                                {{ ($event && $event->id == $eventOption->id) || old('event_id') == $eventOption->id ? 'selected' : '' }}>
                                            {{ $eventOption->name }} - {{ $eventOption->start_date->format('M j, Y') }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('event_id')
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="description" id="description" rows="3" 
                                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                          placeholder="Describe the ticket type..." required>{{ old('description') }}</textarea>
                                @error('description')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <select name="day" id="day" 
                                    ...
                                    <option value="both" {{ old('day') === 'both' ? 'selected' : '' }}>Both Days</option>
                                </select>
                                @error('day')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="time" name="admission_time" id="admission_time" value="{{ old('admission_time', '11:00') }}" 
                            ...
                            <div>
                                ...
                                <input type="number" name="total_quantity" id="total_quantity" value="{{ old('total_quantity') }}" 
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
